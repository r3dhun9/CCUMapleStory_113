...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendSimple("嗨，我是#p9120103# 如果您有 #b#t5152008##k,我將可以幫您隨機整形!\r\n#L2#使用 #b#t5152008##k");
        } else if (status == 1) {
            if (selection == 1) {
                if(cm.getMeso() >= price) {
                    cm.gainMeso(-price);
                ...
                cm.sendYesNo("注意!這是隨機，請問是否要使用 #b#t5152008##k?");
            }
        }
        else if (status == 2){
            cm.dispose();
            if (cm.haveItem(5152008) == true){
                cm.gainItem(5152008, -1);
                cm.setFace(facenew[Math.floor(Math.random() * facenew.length)]);
                cm.sendOk("享受");
            } else {
                cm.sendOk("您貌似沒有#b#t5152008##k..");
            }
        }
    }
}
