...
public class GashaponFactory {
    ...
    public static GashaponFactory getInstance() {
        if (instance == null) {
            System.out.println("【讀取中】 GashaponFactory :::");
    ...
    public void reloadGashapons() {
        ...
        long chanceTotal = 0L;
        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM gashapons;"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
    ...
    public void reloadGashapons(int npcId) {
        ...
        Connection con = DatabaseConnection.getConnection();
        long chanceTotal = 0L;
        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM gashapons WHERE npcId = ?")) {
            ps.setInt(1, npcId);
