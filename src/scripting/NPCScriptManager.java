...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, final int npc, String script) {
        ...
        try {
            ...
            FilePrinter.printError("NPCScriptManager", "Error executing NPC script, NPC ID : " + npc + "." + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
    ...
    public final void action(final MapleClient c, final byte mode, final byte type, final int selection) {
        if (mode != -1) {
            ...
            try {
                ...
                FilePrinter.printError("NPCScriptManager", "Error executing NPC script, NPC ID : " + cm.getNpc() + "." + e);
                dispose(c);
            } finally {
                lock.unlock();
            }
    ...
    public final void startQuest(final MapleClient c, final int npc, final int quest) {
        ...
        try {
            ...
            FilePrinter.printError("NPCScriptManager", "Error executing Quest script. (" + quest + ")..NPCID: " + npc + ":" + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
    ...
    public final void startQuest(final MapleClient c, final byte mode, final byte type, final int selection) {
        ...
        try {
            ...
            FilePrinter.printError("NPCScriptManager", "Error executing Quest script. (" + cm.getQuest() + ")..NPCID: " + cm.getNpc() + ":" + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
